...
def test_next_post_slots_are_mwf_10am_future_within_window():
    slots = next_post_slots(WED_9AM, 4)
    # Today is Wed 9am -> today's 10am still counts, then Fri, Mon, Wed.
    assert [d.weekday() for d in slots] == [2, 4, 0, 2]
    assert all(d > WED_9AM for d in slots)
    assert all(d.hour == 10 and d.minute == 0 for d in slots)
